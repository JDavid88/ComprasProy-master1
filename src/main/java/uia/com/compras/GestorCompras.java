...
public class GestorCompras {
    ...
    //Aqui tienen que ir las otras

    protected HashMap<Object, HashMap<Integer, ArrayList<Cotizacion>>> misSolicitudesCotizacion = new HashMap<Object, HashMap<Integer, ArrayList<Cotizacion>>>();
    ...
    public GestorCompras() throws IOException {
        ...
        try {
			...
			miReporteNS = mapper.readValue(new FileInputStream("F:\\IS\\examen\\ComprasProy-master\\arregloItemsV1.json"), ListaReportesNivelStock.class );
        ...
        for (Entry<Integer, HashMap<Integer, ArrayList<InfoComprasUIA>>> item : miComprador.getSolicitudesOrdenCompraAgrupadosXvendedores().entrySet())
        {
            ...
            for (Entry<Integer, ArrayList<InfoComprasUIA>> soc : nodo.entrySet())
            {
                ...
                mapper.writeValue(new File("F:\\IS\\examen\\ComprasProy-master\\SolicitudOrdenCompra-" + newSolicitud.getName() + ".json"), newSolicitud);
            }
        }
